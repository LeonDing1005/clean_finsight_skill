...
class IndexBuilder:
    ...
    def _load_cache(self):
        ...
        if os.path.exists(self.cache_file_path):
            try:
                ...
                get_logger().warning(f"Could not load cache from {self.cache_file_path}. File might be corrupted: {e}")
                self.cache = {"search": {}, "embeddings": {}} # Reset cache if loading fails
        else:
            get_logger().info(f"No cache file found at {self.cache_file_path}. Starting with empty cache.")
            self.cache = {"search": {}, "embeddings": {}}
    ...
    def _save_cache(self):
        ...
        try:
            ...
            get_logger().warning(f"Could not save cache to {self.cache_file_path}: {e}")
    ...
    async def _get_embeddings_batch(self, batch: List[str], n_retries: int = 3):
        ...
        response = []
        embedding_timeout = int(os.getenv('EMBEDDING_TIMEOUT', '120'))  # seconds per batch
        for attempt in range(n_retries):
            try:
                response = await asyncio.wait_for(
                    self.llm.generate_embeddings(to_compute_texts),
                    timeout=embedding_timeout
                )
                break
            except asyncio.TimeoutError:
                get_logger().warning(
                    f"Embedding API timed out after {embedding_timeout}s "
                    f"(attempt {attempt + 1}/{n_retries}), batch size={len(to_compute_texts)}"
                )
                if attempt == n_retries - 1:
                    get_logger().error(
                        f"Embedding API timed out after {n_retries} attempts. "
                        f"Skipping {len(to_compute_texts)} texts in this batch."
                    )
                    response = []
            except Exception as e:
                get_logger().warning(
                    f"Error getting embeddings (attempt {attempt + 1}/{n_retries}): {e}"
                )
                if attempt == n_retries - 1:
                    get_logger().error(
                        f"Failed to get embeddings after {n_retries} attempts for a batch."
                    )
                    response = []
    ...
    def _save_index(self):
        ...
        if not self.embeddings:
            get_logger().warning("No embeddings to save. Index is empty.")
            return

        # Filter out invalid embeddings (empty, None, or non-list)
        # and ensure all embeddings have the same dimension
        valid_embeddings = []
        expected_dim = None
        for emb in self.embeddings:
            if emb is None or not isinstance(emb, (list, np.ndarray)) or len(emb) == 0:
                get_logger().warning(f"Skipping invalid embedding: {type(emb).__name__} "
                      f"(len={len(emb) if hasattr(emb, '__len__') else 'N/A'})")
                continue
            dim = len(emb)
            if expected_dim is None:
                ...
                get_logger().warning(f"Embedding dimension mismatch (expected {expected_dim}, got {dim}). "
                      f"Padding/truncating to match.")
                if dim < expected_dim:
                    emb = list(emb) + [0.0] * (expected_dim - dim)
                else:
                    emb = list(emb)[:expected_dim]
                valid_embeddings.append(emb)

        if not valid_embeddings:
            get_logger().warning("All embeddings were invalid. Index will not be saved.")
            return

        self.embeddings = valid_embeddings
        get_logger().info(f"Saving index with {len(self.embeddings)} embeddings (dim={expected_dim})")
        ...
        try:
            ...
            get_logger().error(f"Could not save embeddings index to {self.save_file_path}: {e}")
    ...
    def load_index(self):
        ...
        if os.path.exists(self.save_file_path):
            try:
                ...
                get_logger().info(f"Successfully loaded index with {len(self.embeddings)} embeddings from {self.save_file_path}.")
            except Exception as e:
                get_logger().warning(f"Could not load index from {self.save_file_path}. File might be corrupted or in wrong format: {e}")
                self.embeddings = [] # Reset if loading fails
        else:
            get_logger().info(f"No index file found at {self.save_file_path}. Starting with empty index.")

    async def search(self, query: str, top_k: int = 10) -> List[Tuple[int, float]]:
        """Search for similar items in the index."""
        if self.embeddings is None:
            get_logger().warning("Embeddings index is empty. Cannot perform search.")
            return []
        ...
        try:
            ...
            get_logger().error(f"Could not get embedding for query '{query}': {e}")
            return []
        ...
        if distances.size == 0:
            get_logger().warning("No distances computed. Embeddings array might be empty.")
            return []
