...
class AsyncCodeExecutor:
    """
    Compatibility executor for explicitly trusted generated code.

    This is not a security boundary. Run untrusted code in an OS-level sandbox.
    """
           ...
           asyncio.iscoroutinefunction(self.globals['async_main']):
            ...
            try:
                ...
                with redirect_stdout(stdout_capture), redirect_stderr(stderr_capture):
                    ...
                    await asyncio.wait_for(
                        self.globals['async_main'](), timeout=self.exec_timeout,
                    )
            except asyncio.TimeoutError:
                has_error = True
                stderr_capture.write(
