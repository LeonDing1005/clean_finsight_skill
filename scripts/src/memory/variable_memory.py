...
class Memory:
    ...
    def load(self, checkpoint_name: str = 'memory.pkl'):
        ...
        """
        if not self.config.config.get('allow_unsafe_resume', False):
            self.logger.warning(
                "Memory restore is disabled. Enable --resume only for trusted local output directories."
            )
            return False
        target_path = os.path.join(self.save_dir, checkpoint_name)
    ...
    async def get_or_create_agent(
        ...
        **agent_kwargs
        ...
        if resume and not self.config.config.get('allow_unsafe_resume', False):
            self.logger.warning(
                "Agent restore is disabled. Creating a fresh agent instead."
            )
            resume = False
    ...
    def get_collect_data(self, exclude_type: List[str] = []):
        ...
        # NOTE: Do NOT unconditionally exclude DeepSearchResult —
        # it is often the primary data source for A-share/HK markets where
        # structured API tools (akshare) may return sparse results.
        # DeepSearchResult is excluded only when 'deepsearch' is in exclude_type.
        ...
        if exclude_type != []:
            for exclude_type_item in exclude_type:
                ...
                elif exclude_type_item == 'deepsearch':
