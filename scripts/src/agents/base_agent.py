...
class BaseAgent:
    ...
    async def from_checkpoint(
        ...
        **kwargs
        ...
        if not config.config.get('allow_unsafe_resume', False):
            get_logger().warning(
                "Checkpoint restore is disabled. Enable --resume only for trusted local output directories."
            )
            return None
    ...
    async def _handle_code_action(self, action_content: str):
        if not self.enable_code:
            return {
                ...
                "result": "Generated code execution is disabled. Enable it only for trusted inputs in an isolated environment.",
                "continue": True,
            }
