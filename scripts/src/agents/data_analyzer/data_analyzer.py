...
class DataAnalyzer(BaseAgent):
    ...
    async def _draw_chart(self, input_data, run_data: dict, max_iterations: int = 3):
        ...
        for long_chart_name in chart_names:
            if long_chart_name in charts_completed:
                continue
            # Shared environments need isolation; temporarily limit concurrency to 1.
            with Semaphore(1):
                ...
                # ── Eagerly persist to memory so report generator can find images
                # even if later charts fail or time out ──
                if new_chart_name:
                    partial_result = AnalysisResult(
                        title=run_data.get('report_title', ''),
                        content=report_content,
                        image_save_dir=self.image_save_dir,
                        chart_code_mapping={long_chart_name: new_chart_code},
                        chart_name_mapping={long_chart_name: new_chart_name},
                        chart_name_description_mapping={},
                    )
                    self.memory.add_data(partial_result)
                ...
                try:
                    ...
                    self.logger.warning(f"Failed to save charts checkpoint: {e}")
    ...
    async def _generate_and_execute_code(self, conversation_history: list) -> tuple[str | None, str | None]:
        ...
        """
        if not self.enable_code:
            self.logger.warning("Generated code execution is disabled; skipping chart generation")
            return None, None
